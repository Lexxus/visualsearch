(function (factory) {
    if (typeof define === "function" && define.amd) {
        ...
        define("visualsearch/search_input", ["jquery", "underscore", "backbone"], factory);
  ...
  VS.ui.SearchInput = Backbone.View.extend({
    ...
    setupAutocomplete: function() {
      ...
      this.box.autocomplete('widget').addClass('VS-interface ' +
        this.options.inputAutocompleteClassName);
    },
    ...
    autocompleteValues: function(req, resp) {
      ...
      this.app.options.callbacks.facetMatches(function(prefixes, options) {
        ...
        var matcher = new RegExp((!options.matchAny ? '^' : '') + re, 'i');
    ...
    keydown: function(e) {
      ...
      if (key == 'left') {
        ...
        if (value.length) {
          ...
          var position = this.options.position + (remainder ? 1 : 0);
          if (value != remainder) {
            this.app.searchBox.value(this.app.searchBox.value());
            this.app.searchBox.focusSearch(e);
          }
        } else {
        ...
        view = this.app.searchBox.inputViews[this.app.searchBox.inputViews.length - 1];
        view.setCursorAtEnd(-1);
      } else if (key == 'home') {
        view = this.app.searchBox.inputViews[0];
        view.setCursorAtEnd(-1);
      }
