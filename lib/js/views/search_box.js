(function (factory) {
    if (typeof define === "function" && define.amd) {
        ...
        define("visualsearch/search_box", ["jquery", "underscore", "backbone"], factory);
  ...
  VS.ui.SearchBox = Backbone.View.extend({
    ...
    searchEvent: function(e) {
      var query = this.value(),
        hasNewTextFacet = this.$('.search_input input').val();

      this.focusSearch(e);
      this.value(query);

      if (!hasNewTextFacet || this.app.options.autosearch) {
    ...
    renderFacet: function(facet, position) {
      var view = new VS.ui.SearchFacet({
        ...
        model: facet,
        order: position,
        facetAutocompleteClassName: this.options.app.options.className.facetAutocomplete
      });
    ...
    renderSearchInput: function() {
      var input = new VS.ui.SearchInput({
        ...
        inputAutocompleteClassName: this.options.app.options.className.inputAutocomplete
      });
    ...
    _maybeDisableFacets: function(e) {
      ...
      if (this.flags.allSelected && (key === 'backspace' || key === 'delete')) {
        e.preventDefault();
        this.clearSearch(e);
        return false;
